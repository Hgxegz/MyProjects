...
string address, url;//"800 N State College Blvd, Fullerton, CA 92831";
int choice;
...
if (choice == 1){
  ...
  cin.ignore();
  getline(cin, address);
  url = buildGeocodeRequestUrl(address);
} else if (choice == 2){
  string latlng;
  string lat;
  string lng;
  ...
  getline(cin,lng);
  latlng = lat +","+ lng;
  url = buildReverseGeocodeRequestUrl(latlng);
} else if (choice == 0){
  cout << "Bye!";
  return 0;
}
...
if (curl) {
  ...
  if (choice == 2){
    cout << getAddress(responseMessage) << endl;
  }
